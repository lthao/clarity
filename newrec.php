...
<body>
	...
	<?php
		...
		$recentLog = "SELECT * FROM clarity WHERE what='$name' AND username='$username'";
		//echo $recentlog, "<br/>";
		$recentLogResult = mysql_query($recentLog);
		$newRow = mysql_fetch_array($recentLogResult); 
		echo "<p>Your log of \"", $newRow['what'], "\" has these tags:";
		$name = $newRow['what'];
		$tag1 = $newRow['tag1'];
		$tag2 = $newRow['tag2'];
		$tag3 = $newRow['tag3'];
		...
		echo"</br>";
		if ($num!=100) {
			echo "<p>Using these tags we recommend <br/> these top ", $num, " activities: <p/>";
		} else {
